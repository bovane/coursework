package com.student.cw.lecture15;
...
public class NumberServer {
    ...
    public NumberServer( String host, int port ) {
        ...
        try {
            socket = new MulticastSocket(); // Port set at run time
            socket.setTimeToLive( ttl );    // TTL for all packets
        }
    ...
    public static void main( String[] args ) {
        NumberServer server = new NumberServer( "228.3.4.5", 4446 );
        server.runServer();
    }
}
